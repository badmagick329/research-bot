...
const nonSensitiveConfigSchema = z.object({
  ...
  THESIS_KPI_CARRY_FORWARD_MAX_AGE_DAYS: z.coerce.number().int().min(1).max(365),
  THESIS_CORE_KPI_MIN_REQUIRED: z.coerce.number().int().min(1).max(10),
  THESIS_GRACE_ALLOW_ON_SECTOR_WEAKNESS: z.coerce.boolean(),
  ...
  ALPHA_VANTAGE_DAILY_REQUEST_CAP: z.coerce.number().int().positive(),
